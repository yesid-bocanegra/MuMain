                                      ...
                                      wchar_t* pszName, wchar_t* pszPrice, wchar_t* pszPeriod)
{
    ...
    m_iNumNoticeLine = ::DivideStringByPixel(&m_szNotice[0][0], NUM_LINE_CMB, MAX_TEXT_LENGTH, GlobalText[2898],
...
{
    auto* pOwnMsgBox = dynamic_cast<CMsgBoxIGSBuyConfirm*>(pOwner);
    SocketClient->ToGameServer()->SendCashShopItemBuyRequest(pOwnMsgBox->m_iPackageSeq, pOwnMsgBox->m_iDisplaySeq,
                                                             pOwnMsgBox->m_iPriceSeq, pOwnMsgBox->m_wItemCode,
                                                             pOwnMsgBox->m_iCashType, 0);
...
void CMsgBoxIGSBuyConfirm::RenderTexts()
{
    ...
    g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_TITLE_POS_Y, GlobalText[2896], IMAGE_IGS_FRAME_WIDTH, 0,
                              RT3_SORT_CENTER);
    g_pRenderText->SetFont(g_hFont);
    g_pRenderText->RenderText(GetPos().x, GetPos().y + IGS_TEXT_QUESTION_POS_Y, GlobalText[2897], IMAGE_IGS_FRAME_WIDTH,
                              0, RT3_SORT_CENTER);
