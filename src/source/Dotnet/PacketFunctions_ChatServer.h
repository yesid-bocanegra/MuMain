...
public:
    ...
    /// <param name="token">A token (integer number), formatted as string and "encrypted" with the 3-byte XOR key (FC CF
    /// AB).</param> <param name="tokenByteLength">The length of <paramref name="token"/> in bytes. <remarks> Is sent by
    /// the client when: This packet is sent by the client after it connected to the server, to authenticate itself.
    /// Causes reaction on server side: The server will check the token. If it's correct, the client gets added to the
    /// requested chat room.
    /// </remarks>
    ...
    /// Causes reaction on client side: The client will add the client in its list (if over 2 clients are connected to
    /// the same room), or show its name in the title bar.
    /// </remarks>
    ...
    /// </summary>
    /// <remarks>
    /// Is sent by the client when: This packet is sent by the client when it leaves the chat room, before the
    /// connection closes. Causes reaction on server side: The server will remove the client from the chat room,
    /// notifying the remaining clients.
    /// </remarks>
    ...
    /// Causes reaction on client side: The client will remove the client from its list, or mark its name in the title
    /// bar as offline.
    /// </remarks>
    ...
    /// Causes reaction on server side: The server will keep the connection and chat room intact as long as the clients
    /// sends a message in a certain period of time.
    /// </remarks>
    void SendKeepAlive();
};