...
{
    return (Position[0] >= 16100.f && Position[0] <= 19000.f && Position[1] >= 18900.f && Position[1] <= 21700.f);
}
inline bool InBattleCastle3(vec3_t Position)
{
    return (Position[0] >= 4700.f && Position[0] <= 14800.f && Position[1] >= 5300.f && Position[1] <= 23900.f);
}
...
bool CollisionEffectToObject(OBJECT* eff, float Range, float RangeZ, bool bCollisionGround,
                             bool bRealCollision = false);
