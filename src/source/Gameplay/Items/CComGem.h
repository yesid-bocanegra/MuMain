...
inline void SetGem(char gem)
{
    m_cGemType = gem;
}
inline void SetComType(char type)
{
    m_cComType = type;
}
...
inline bool isComMode()
{
    return (m_bType == ATTACH);
}

__inline int Check_Jewel_Unit(int _nJewel, bool _bModel = false)
{
    return Check_Jewel(_nJewel, 1, _bModel);
}
__inline int Check_Jewel_Com(int _nJewel, bool _bModel = false)
{
    return Check_Jewel(_nJewel, 2, _bModel);
}
__inline bool isCompiledGem(const ITEM* p)
{
    return (Check_Jewel_Com(p->Type) != NOGEM ? true : false);
}
inline bool isAble()
{
    return (m_cState == STATE_READY);
}
} // namespace COMGEM
