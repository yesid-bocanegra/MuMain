...
// It's 5, because the number of equippable items is 12, but one is a wing and one is a pet. To make any set, you need
// at least 2 items.
...
constexpr auto MAX_OPTIONS_PER_ITEM_SET =
    MAX_ITEM_SET_STANDARD_OPTION_COUNT * MAX_ITEM_SET_STANDARD_OPTION_PER_ITEM_COUNT + MAX_ITEM_SET_FULL_OPTION_COUNT +
...
struct ITEM_SET_OPTION
{
    ...
    // These 2-element-arrays are somehow strange. Only the first element is actually used, I found no use case for the
    // second one. Theoretically Webzen planned to have 2 options per item, but never implemented it.
...
struct SET_OPTION
{
    ...
    bool IsFullOption;
    bool IsExtOption;
    bool FulfillsClassRequirement; // If the option requires a specific class and the character fulfills that.
                                   // //m_bySetOptionList[x][1]
...
private:
    ...
    static void TryAddSetOption(std::uint8_t option, int value, int optionIndex, SET_SEARCH_RESULT_OPT& set,
    ...
    static std::uint8_t RenderSetOptionList(const SET_SEARCH_RESULT_OPT& set, std::uint8_t textIndex,
    ...
    void AddStatsBySetOptions(std::uint16_t* Strength, std::uint16_t* Dexterity, std::uint16_t* Energy,
                              std::uint16_t* Vitality, std::uint16_t* Charisma)
...
public:
    ...
    void getAllAddOptionStatesbyCompare(std::uint16_t* Strength, std::uint16_t* Dexterity, std::uint16_t* Energy,
                                        std::uint16_t* Vitality, std::uint16_t* Charisma,
                                        std::uint16_t iCompareStrength, std::uint16_t iCompareDexterity,
                                        std::uint16_t iCompareEnergy, std::uint16_t iCompareVitality, std::uint16_t iC);

    void getAllAddStateOnlyAddValue(std::uint16_t* AddStrength, std::uint16_t* AddDexterity, std::uint16_t* AddEnergy,
                                    std::uint16_t* AddVitality, std::uint16_t* AddCharisma)
