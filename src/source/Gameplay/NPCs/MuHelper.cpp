...
int CMuHelper::Buff()
{
    ...
    if (m_config.bSupportParty && g_pPartyManager->IsPartyActive())
    {
        ...
        if (pChar != NULL && pMember->Map == gMapManager.WorldActive &&
...
int CMuHelper::BuffTarget(CHARACTER* pTargetChar, ActionSkillType iBuffSkill)
{
    ...
    if ((iBuffSkill == AT_SKILL_DEFENSE || iBuffSkill == AT_SKILL_DEFENSE_STR ||
    ...
    if ((iBuffSkill == AT_SKILL_SOUL_BARRIER || iBuffSkill == AT_SKILL_SOUL_BARRIER_STR ||
    ...
    if ((iBuffSkill == AT_SKILL_SWELL_LIFE || iBuffSkill == AT_SKILL_SWELL_LIFE_STR ||
    ...
    if ((iBuffSkill == AT_SKILL_EXPANSION_OF_WIZARDRY || iBuffSkill == AT_SKILL_EXPANSION_OF_WIZARDRY_STR ||
         iBuffSkill == AT_SKILL_EXPANSION_OF_WIZARDRY_MASTERY) &&
        (!g_isCharacterBuff((&pTargetChar->Object), eBuff_SwellOfMagicPower)))
    {
        return SimulateSkill(iBuffSkill, false, pTargetChar->Key);
    }

    if ((iBuffSkill == AT_SKILL_ADD_CRITICAL || iBuffSkill == AT_SKILL_ADD_CRITICAL_STR1 ||
         iBuffSkill == AT_SKILL_ADD_CRITICAL_STR2 || iBuffSkill == AT_SKILL_ADD_CRITICAL_STR3) &&
    ...
    if ((iBuffSkill == AT_SKILL_ALICE_THORNS) && (!g_isCharacterBuff((&pTargetChar->Object), eBuff_Thorns)))
    {
        return SimulateSkill(iBuffSkill, false, pTargetChar->Key);
    }
...
ActionSkillType CMuHelper::SelectAttackSkill()
{
    ...
    {
        if ((m_config.aiSkillCondition[1] & ON_TIMER) && m_config.aiSkillInterval[1] != 0 &&
    ...
    {
        if ((m_config.aiSkillCondition[2] & ON_TIMER) && m_config.aiSkillInterval[2] != 0 &&
