...
public:
    CSBaseMatch()
        : m_byMatchEventType(0), m_iMatchCountDownType(TYPE_MATCH_NONE),
          m_matchCountDownStart(MatchClock::time_point{}), m_byMatchType(0), m_iMatchTimeMax(-1), m_iMatchTime(-1),
          m_iMaxKillMonster(-1), m_iKillMonster(-1), m_iNumResult(0), m_iMyResult(0), m_MatchResult{}
    {
    }
    virtual ~CSBaseMatch() {};
    ...
    virtual void SetMatchResult(const int iNumDevilRank, const int iMyRank, const MatchResult* pMatchResult,
                                const int Success = false) = 0;
