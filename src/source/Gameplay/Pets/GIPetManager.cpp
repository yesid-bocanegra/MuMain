...
bool RenderPetCmdInfo(int sx, int sy, int Type)
{
    ...
    if (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD)
    {
        ...
        mu_swprintf(TextList[TextNum], GlobalText[1219 + cmdType]);
        TextNum++;
        SkipNum++;
        ...
        case PET_CMD_DEFAULT:
            mu_swprintf(TextList[TextNum], GlobalText[1223]);
            TextNum++;
            SkipNum++;
            break;
        case PET_CMD_RANDOM:
            mu_swprintf(TextList[TextNum], GlobalText[1224]);
            TextNum++;
            SkipNum++;
            break;
        case PET_CMD_OWNER:
            mu_swprintf(TextList[TextNum], GlobalText[1225]);
            TextNum++;
            SkipNum++;
            break;
        case PET_CMD_TARGET:
            mu_swprintf(TextList[TextNum], GlobalText[1226]);
            TextNum++;
            SkipNum++;
            break;
        }
...
void SetPetItemConvert(ITEM* ip, PET_INFO* pPetInfo)
{
    if (ip->Type == ITEM_DARK_HORSE_ITEM)
    {
        ...
        if (Index == 0)
        {
            ip->SpecialValue[ip->SpecialNum] =
                static_cast<std::uint8_t>((5 + (CharacterAttribute->Dexterity / 20) + pPetInfo->m_wLevel * 2) & 0xFF);
            ip->Special[ip->SpecialNum] = AT_SET_OPTION_IMPROVE_DEFENCE;
            ip->SpecialNum++;
        }
        else
        {
            ip->SpecialValue[Index] =
                static_cast<std::uint8_t>((5 + (CharacterAttribute->Dexterity / 20) + pPetInfo->m_wLevel * 2) & 0xFF);
...
bool RenderPetItemInfo(int sx, int sy, ITEM* pItem, int iInvenType)
{
    ...
    {
        appendLine(TEXT_COLOR_WHITE, false, true, GlobalText[203], pPetInfo->m_wDamageMin, pPetInfo->m_wDamageMax,
    ...
    const int ownershipColor =
        (gCharacterManager.GetBaseClass(Hero->Class) == CLASS_DARK_LORD) ? TEXT_COLOR_WHITE : TEXT_COLOR_DARKRED;
