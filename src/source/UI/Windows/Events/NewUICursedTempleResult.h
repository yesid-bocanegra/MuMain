...
inline void CNewUICursedTempleResult::SetPos(int x, int y)
{
    m_Pos.x = x;
    m_Pos.y = y;
}

inline void CNewUICursedTempleResult::SetMyTeam(SEASON3A::eCursedTempleTeam myteam)
{
    m_MyTeam = myteam;
}

inline const POINT& CNewUICursedTempleResult::GetPos() const
{
    return m_Pos;
}

inline float CNewUICursedTempleResult::GetLayerDepth() //. 5.0f
{
    return 10.2f;
}
}; // namespace SEASON3B
