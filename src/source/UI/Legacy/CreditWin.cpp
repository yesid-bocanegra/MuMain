...
template <typename T> short IncreaseAlpha(short alpha, T ratio)
{
...
template <typename T> short DecreaseAlpha(short alpha, T ratio)
{
    const double delta = 255.0 * std::clamp(static_cast<double>(ratio), 0.0, 1.0);
...
CCreditWin::CCreditWin()
    : m_eIllustState(HIDE), m_illustElapsed(DurationMs::zero()), m_byIllust(0), m_illustPaths(kIllustPaths),
      m_font(nullptr, &FontDeleter), m_nNowIndex(0), m_nNameCount(0), m_anTextIndex{}, m_aeTextState{},
...
void CCreditWin::Create()
{
    ...
    m_aSpr[CRW_SPR_LOGO].Create(290, 41, BITMAP_LOG_IN + 14, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, fScaleX,
                                fScaleY);
    ...
    HFONT fontHandle =
        CreateFont(nFontSize, 0, 0, 0, FW_BOLD, 0, 0, 0, DEFAULT_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS,
                   NONANTIALIASED_QUALITY, DEFAULT_PITCH | FF_DONTCARE, GlobalText[0][0] ? GlobalText[0] : NULL);
...
void CCreditWin::SetPosition()
{
    ...
    m_aSpr[CRW_SPR_DECO].SetPosition(rInput.GetScreenWidth() - m_aSpr[CRW_SPR_DECO].GetWidth(),
                                     nBaseY - m_aSpr[CRW_SPR_DECO].GetHeight());

    for (int i = CRW_SPR_TXT_HIDE0; i <= CRW_SPR_TXT_HIDE2; ++i)
        m_aSpr[i].SetPosition(0, 42 * (i - CRW_SPR_TXT_HIDE0));

    m_btnClose.SetPosition(m_aSpr[CRW_SPR_DECO].GetXPos() + 122, m_aSpr[CRW_SPR_DECO].GetYPos() + 63);
}
...
void CCreditWin::LoadIllust()
{
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        m_aSpr[i].Create(400, 400, BITMAP_TEMP + i, 0, NULL, 0, 0, false, SPR_SIZING_DATUMS_LT, fScaleX, fScaleY);
        m_aSpr[i].SetAlpha(0);
        m_aSpr[i].Show(true);
    }
