...
#include "NewUIBase.h"
#include "ThirdParty/xstreambuf.h"
...
public:
    ...
    public:
        TContainer() : m_pObj(NULL), m_InstState(INSTANCE_NEW)
        {
            m_pObj = new T;
        } //. create instance
...
template <class _M> class TMsgBoxLayout : public IMsgBoxLayout
{
...
template <class _L, class _M> bool CreateMessageBox(const TMsgBoxLayoutContainer<_L>& container, _M** ppMsgBox)
{
    if (false == container.Create()) //. MessageBox Layout
        return false;
    if (ppMsgBox)
